...
> If you need help, you can create an anonymized support ticket from the app or through MCP. For a failed run, `/support [run-id]` includes run context in a prefilled GitHub issue.
...
The default endpoint is `http://127.0.0.1:39387/mcp`. Change the port under **Settings -> Local MCP server**, save, and restart. `INXM_MCP_ONLY=1 inxm-local` is useful when the native window cannot start.
